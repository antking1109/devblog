  ...
  if (isset($_GET['id'],$_GET['name']) && filter_var($_GET['id'],FILTER_VALIDATE_INT,array('min_range'=>1))) {
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
      if(isset($_POST['rad4']) && $_POST['rad4'] == 'co'){
        $q = "DELETE FROM pages WHERE page_id={$id}";
            ...
            <?php
               if(isset($mess)) {
                  echo "<div class='form-group'>
                          {$mess}<br>
                          <button class='btn btn-space btn-default'><i class='icon icon-left mdi mdi-globe'></i> <a href='view_pages.php'>Quay Lại: Quản Lý Bài Viết</a></button>
                        </div>";
                  exit;
              }
            ?>
              ...
              <div class="form-group">
                      <label class="col-sm-3 control-label">Bạn muốn xóa bài viết: <b><?php if(isset($name)) echo $name; ?></b> ?</label>
                      <div class="col-sm-6">
